  ...
  function bullet(g) {
    g.each(function(d, i) {
      var rangez = d.ranges.slice(),
        ...
        //   rangez.push('rmax');
        var min = rangez[0];
        var max = rangez[rangez.length-1];

      // Compute the new x-scale.
      var x1 = scaleBand().domain(rangez)
      .range([0, width]);

      var markx1 = markerX(x1);

      var color_scale = scaleLinear().domain([x1(min),x1(rangez[Math.floor( rangez.length/2)]),x1(max)]).range(d.colors);
     

      // Derive width-scales from the x-scales.
      var rw1 = rectWidth(x1,min);
      var mw1 = measureWidth(x1,min);
      ...
      range.enter().append("rect")
          ...
          .attr("height", height)
          .attr("x",  0)
          .style("fill", function(d){ return color_scale(x1(d));})
          .style("opacity", 0.4); 
...
function rectWidth(x,first) {
  var x0 = x(first);
  var step = x.step();
  return function(d) {
    return Math.abs(x(d) - x0+step);
  };
}

function measureWidth(x,first) {
  var x0 = x(first);
  var step = x.step()/2;
  return function(d) {
    return Math.abs(x(d) - x0+step);
